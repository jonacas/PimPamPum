					...
					for(int cargas = 0; cargas < GlobalData.VALORES_CARGAS; cargas++)
					{
						...
						for (int powerUp = 0; powerUp < GlobalData.VALORES_ESCUDO; powerUp++) 
						{
							...
							for (int distanciaEnemigo = 0; distanciaEnemigo < GlobalData.VALORES_DISTANCA_ENEMIGO; distanciaEnemigo++) 
							{
								...
								{
									matriz[fila][columna][salud][cargas][powerUp][distanciaEnemigo][saludEnemigo] = new int[GlobalData.VALORES_ESCUDO_ENEMIGO][];
									for(int escudoEnemigo = 0; escudoEnemigo < GlobalData.VALORES_ESCUDO_ENEMIGO; escudoEnemigo++)
									{
										...
										for (int cargasEnemigo = 0; cargasEnemigo < GlobalData.CARGAS_ENEMIGO; cargasEnemigo++) {
											...
											if (salud == 0) {
												//si hay empate
												if (saludEnemigo == 0)
													matriz [fila] [columna] [salud] [cargas] [powerUp] [distanciaEnemigo] [saludEnemigo] [escudoEnemigo] [cargasEnemigo] = -50;
												else
